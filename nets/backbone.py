...
#??
def autopad(k , p=None): # what does 'p=None' mean
    if p is None:
...
class Backbone(nn.Module):
    def __init__(self, transition_channels, block_channels, n, phi, pretrained=False):
        ...
        #-----------------------------------------------#
        # i make a mistake in input of first Conv
        #   Conv(block_channels, 32, 3, 1),
        #   Conv(transition_channels, transition_channels*2, 3, 2),
        #   Conv(transition_channels*2, transition_channels*2, 3, 1)
        #-----------------------------------------------#
            Conv(3, transition_channels, 3, 1), # transition_channels == 32
            Conv(transition_channels, transition_channels * 2, 3, 2),
            Conv(transition_channels * 2, transition_channels * 2, 3, 1),
        )
        ...
        if pretrained:
            ...
            self.load_state_dict(checkpoint, strick=False)
    ...
    def forward(self, x):
        ...
        x = self.stage2(x)
        #-----------------------------------------------#
        #   the shape of feat1 is 80*80*512
        #-----------------------------------------------#
        x = self.stage3(x)
        feat1 = x
        #-----------------------------------------------#
        #   the shape of feat2 is 40*40*1024
        #-----------------------------------------------#
        x = self.stage4(x)
        feat2 = x
        #-----------------------------------------------#
        #   the shape of feat3 is 20*20*1024
        #-----------------------------------------------#
        x = self.stage5(x)
        feat3 = x
